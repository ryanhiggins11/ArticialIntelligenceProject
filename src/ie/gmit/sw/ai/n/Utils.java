...
public class Utils {
	...
	//Normalize the input variables to values within the range [lower..upper]
    public static double[] normalize(double[] vector, double lower, double upper) { //normalize = squash values
        double[] normalized = new double[vector.length];
        double max = Arrays.stream(vector).max().getAsDouble(); //get row and vector and get max and min values
    ...
    //Softmax transforms a vector into a probability distribution = e(xi)/(e(xi)/sum(e(xi->j))) 
    public static double[] softmax(double[] vector) {
    	double[] softmax = new double[vector.length];
    	
    	double total = Arrays.stream(vector).map(n -> Math.exp(n)).reduce(0, Double::sum);
    	for (int i = 0; i < softmax.length; i++) softmax[i] = Math.exp(vector[i]) / total;
    ...
    public static int getMaxIndex(double[] vector){
		double max = Double.MIN_VALUE; //give max index
		int maxIndex = -1;
		...
		return maxIndex; //index no Corresponds to the classification
    }
}